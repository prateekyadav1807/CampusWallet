...
/* -------------------------------------------------------------------
   STAT CARD with gradient border + sparkline
   ------------------------------------------------------------------- */
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
        <div>
          <p style={{ fontSize: 11, fontWeight: 500, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>
            {label}
          </p>
          <p style={{ fontSize: 26, fontWeight: 700, color: '#fff', letterSpacing: '-0.03em', fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
            {value}
          </p>
        </div>
        <div style={{
          ...
          flexShrink: 0, boxShadow: `0 4px 16px ${iconBg}55`,
        }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ position: 'relative', width: 130, height: 130 }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: 22, fontWeight: 700, color, letterSpacing: '-0.03em', lineHeight: 1 }}>
            {pct}%
          </p>
          <p style={{ fontSize: 10, color: '#64748B', marginTop: 2 }}>used</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, width: '100%' }}>
        {[
          ...
          <div key={s.l} style={{ padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <p style={{ fontSize: 10, color: '#64748B', marginBottom: 2 }}>{s.l}</p>
            <p style={{ fontSize: 13, fontWeight: 600, color: s.c, fontVariantNumeric: 'tabular-nums' }}>{s.v}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ flexShrink: 0, position: 'relative', width: 120, height: 120 }}>
            ...
            <Tooltip content={({ active: a, payload: p }) => {
              ...
              return (
                <div style={{ ...glassCard(), padding: '8px 12px', fontSize: 11 }}>
                  ...
                  <p style={{ color: d.payload.fill }}>{formatCurrency(d.value)} � {((d.value / total) * 100).toFixed(0)}%</p>
                </div>
              );
            }} />
          </PieChart>
        ...
        {active !== null && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: PIE_COLORS[active % PIE_COLORS.length] }}>
        ...
        {data.slice(0, 5).map((d, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'default' }}
              ...
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: PIE_COLORS[i % PIE_COLORS.length], flexShrink: 0, boxShadow: `0 0 6px ${PIE_COLORS[i % PIE_COLORS.length]}66` }} />
              <span style={{ flex: 1, fontSize: 11, color: active === i ? '#fff' : '#94A3B8', transition: 'color 0.15s', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.name}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{ position: 'relative', width: 88, height: 88, flexShrink: 0 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <p style={{ fontSize: 20, fontWeight: 700, color, letterSpacing: '-0.03em', lineHeight: 1 }}>{score}</p>
            <p style={{ fontSize: 9, color: '#64748B' }}>/ 100</p>
          </div>
        </div>
        <div>
          <p style={{ fontSize: 18, fontWeight: 700, color, letterSpacing: '-0.02em' }}>{label}</p>
          <p style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>Financial health score</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
            <Shield size={11} color={color} />
            <span style={{ fontSize: 11, color: '#64748B' }}>Based on savings & budget</span>
          </div>
        </div>
      </div>
        ...
        {factors.map(f => (
          <div key={f.l}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: f.c }}>{f.v}</span>
            </div>
...
function AIInsightCard({ insight, delay = 0 }) {
  ...
  return (
    ...
    >
      <span style={{ fontSize: 16, flexShrink: 0, marginTop: 1 }}>{insight.emoji}</span>
      ...
      <span style={{
        ...
        padding: '2px 6px', borderRadius: 4, background: `${c}22`, color: c, flexShrink: 0,
      }}>
        {insight.priority}
      </span>
    </motion.div>
...
function UpcomingPayments() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const subs = (subR.data.renewals ?? []).map(s => ({
          id: s._id, name: s.name, amount: s.amount, date: s.nextRenewalDate, emoji: '??',
        }));
        const fees = (feeR.data.fees ?? []).slice(0, 3).map(f => ({
          id: f._id, name: f.title, amount: f.totalAmount - f.paidAmount, date: f.dueDate, emoji: '??',
        }));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {items.length === 0 ? (
        <p style={{ textAlign: 'center', padding: '24px 0', color: '#64748B', fontSize: 13 }}>
        ...
        return (
          <motion.div key={item.id}
            ...
            style={{
              ...
              background: urgent ? 'rgba(245,158,11,0.08)' : 'rgba(255,255,255,0.03)',
              border: urgent ? '1px solid rgba(245,158,11,0.2)' : '1px solid rgba(255,255,255,0.05)',
...
function QuickAdd({ onAdded }) {
  ...
  const [form, setForm]   = useState({
    title: '', amount: '', category: 'Mess / Food',
  ...
  return (
    <motion.form onSubmit={submit}
      ...
      style={{ ...glassCard(), padding: '14px 16px', display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'flex-end' }}>
      {[
        { label: 'What?',    k: 'title',    type: 'text',   ph: 'Canteen, transport�', flex: '1 1 160px' },
        { label: '?',        k: 'amount',   type: 'number', ph: '0.00',                 flex: '0 0 100px' },
        { label: 'Date',     k: 'date',     type: 'date',   ph: '',                     flex: '0 0 140px' },
      ].map(f => (
        <div key={f.k} style={{ flex: f.flex, minWidth: 0 }}>
          ...
          <input autoFocus={f.k === 'title'} type={f.type} placeholder={f.ph} value={form[f.k]}
            ...
            style={{ width: '100%', height: 34, padding: '0 10px', borderRadius: 6, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)', color: '#fff', fontSize: 13, outline: 'none' }} />
        </div>
      ))}
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button type="button" onClick={() => setOpen(false)} style={{ height: 34, width: 34, borderRadius: 6, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)', cursor: 'pointer', color: '#94A3B8', fontSize: 16 }}>�</button>
      </div>
    </motion.form>
  );
}
...
function SectionHeader({ title, subtitle, action }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
      <div>
        <p style={{ fontSize: 14, fontWeight: 600, color: '#fff', letterSpacing: '-0.02em' }}>{title}</p>
  ...
  if (loading) return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {[1,2,3,4].map(i => (
          <div key={i} style={{ ...glassCard(), padding: 20, height: 140 }}>
            <div className="skeleton" style={{ height: 12, width: 80, marginBottom: 12 }} />
            <div className="skeleton" style={{ height: 28, width: 110, marginBottom: 16 }} />
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      >
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <p style={{ fontSize: 22, fontWeight: 700, color: '#fff', letterSpacing: '-0.03em' }}>
              Financial Overview
            </p>
            <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 99, background: 'rgba(20,184,166,0.15)', border: '1px solid rgba(20,184,166,0.3)', color: '#14B8A6', fontWeight: 600 }}>
      ...
      {s?.budgetUsedPercentage >= 90 && s.monthlyBudget > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderRadius: 10, marginBottom: 20, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)' }}>
          <Zap size={15} color={DANGER} />
          <p style={{ fontSize: 13, color: DANGER, flex: 1 }}>
            <strong>{s.budgetUsedPercentage}% of budget used</strong> � {formatCurrency(Math.max(0, s.remainingBudget))} remaining
          </p>
          <Link to="/budget" style={{ fontSize: 12, color: DANGER, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 20 }}>
        <StatCard i={0} label="Current Balance" value={formatCurrency(s?.currentBalance ?? 0)}
          icon={Wallet} iconBg="linear-gradient(135deg, #0F766E, #14B8A6)"
          sparkData={savSpark} sparkColor={ACCENT} sub="All-time net" />
        <StatCard i={1} label="Monthly Income" value={formatCurrency(s?.totalIncome ?? 0)}
          icon={TrendingUp} iconBg="linear-gradient(135deg, #15803D, #22C55E)"
          sparkData={incSpark} sparkColor={SUCCESS}
          trend={delta(cur.income ?? 0, prev.income ?? 0)} sub="vs last month" />
        <StatCard i={2} label="Monthly Expenses" value={formatCurrency(s?.totalExpenses ?? 0)}
          icon={TrendingDown} iconBg="linear-gradient(135deg, #B91C1C, #EF4444)"
          sparkData={expSpark} sparkColor={DANGER}
          trend={delta(cur.expenses ?? 0, prev.expenses ?? 0)} sub="vs last month" />
        <StatCard i={3}
          label="Savings Rate"
          value={s?.totalIncome > 0 ? `${Math.round((s.totalSavings / s.totalIncome) * 100)}%` : '�'}
          icon={PiggyBank} iconBg="linear-gradient(135deg, #1D4ED8, #3B82F6)"
          sparkData={savSpark} sparkColor={INFO}
          sub={s?.totalSavings ? `${formatCurrency(s.totalSavings)} saved` : 'No income yet'} />
      </div>

      {/* -- Row 2: Chart + Budget + Health ---------------------- */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 240px 240px', gap: 14, marginBottom: 20 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          />
          {trend.length < 2 ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 180, color: '#64748B', fontSize: 13 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.36 }}
          ...
          {!budget || !s?.monthlyBudget ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '16px 0' }}>
              ...
              <Link to="/budget" style={{ fontSize: 12, fontWeight: 600, color: ACCENT, padding: '6px 14px', borderRadius: 6, background: 'rgba(20,184,166,0.12)', border: '1px solid rgba(20,184,166,0.2)' }}>
                Set Budget
              </Link>
            </div>
          ) : (
            <BudgetRing
              pct={s.budgetUsedPercentage ?? 0}
              spent={s.totalExpenses ?? 0}
              total={s.monthlyBudget}
              remaining={s.remainingBudget ?? 0}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.42 }}
          style={glassCard({ padding: 20 })}>
          <SectionHeader title="Health Score" subtitle="Financial fitness"
            action={<Brain size={14} color="#475569" />}
          />
          <HealthScore s={s} />
        </motion.div>
      </div>

      {/* -- Row 3: Transactions + Spending Donut --------------- */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 14, marginBottom: 20 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.46 }}
          ...
          <SectionHeader title="Recent Transactions" subtitle={`${recentTx.length} latest`}
            action={<Link to="/transactions" style={{ fontSize: 11, color: ACCENT, display: 'flex', alignItems: 'center', gap: 3 }}>See all <ChevronRight size={11} /></Link>}
          />
              ...
              {recentTx.map((tx, i) => {
                ...
                return (
                  <motion.div key={tx._id}
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 10px', borderRadius: 8, transition: 'background 0.15s', cursor: 'default' }}
                    onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.04)'}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                  >
                    <div style={{ width: 32, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isExp ? 'rgba(239,68,68,0.10)' : 'rgba(34,197,94,0.10)', flexShrink: 0, fontSize: 14 }}>
                      {isExp ? (cfg.emoji ?? '??') : '??'}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 500, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tx.title}</p>
                      <p style={{ fontSize: 11, color: '#64748B', marginTop: 1 }}>
                        {isExp ? (tx.category ?? 'Expense') : (tx.type ?? 'Income')} � {formatDate(tx.date, 'dd MMM')}
                      </p>
                    </div>
                    <p style={{ fontSize: 13, fontWeight: 600, color, flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          style={glassCard({ padding: 20 })}>
          <SectionHeader title="Spending Categories" subtitle="This month's breakdown"
      ...
      {/* -- Row 4: AI Insights + Upcoming Payments -------------- */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 14 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.54 }}
          ...
          <SectionHeader title="AI Insights"
            subtitle="Smart analysis of your spending"
            action={
              <Link to="/insights" style={{
                display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 6,
                background: 'rgba(20,184,166,0.10)', border: '1px solid rgba(20,184,166,0.2)', fontSize: 11, color: ACCENT, fontWeight: 600,
              }}>
          ...
          {insights.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {insights.map((ins, i) => (
                <AIInsightCard key={i} insight={ins} delay={0.54 + i * 0.07} />
              ))}
            </div>
          )}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.58 }}
          ...
          <SectionHeader title="Upcoming Payments" subtitle="Next 14 days"
            action={<Link to="/recurring-payments" style={{ fontSize: 11, color: ACCENT, display: 'flex', alignItems: 'center', gap: 3 }}>View all <ChevronRight size={11} /></Link>}
          />
          <UpcomingPayments />
        </motion.div>
      </div>
