...
export default function Register() {
  ...
  const Field = ({ label, k, type = 'text', icon: Icon, placeholder, required, children }) => (
    <div>
      <label className="form-label">
        {label}{required && <span className="text-teal-400 ml-0.5">*</span>}
      </label>
      <div className="relative">
        {Icon && <Icon size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: 'var(--text-label)' }} />}
        {children ?? (
    ...
    <AuthLayout title="Join CampusWallet 🎓" subtitle="Your student finance companion — free forever">
      ...
      <div className="flex items-center gap-2 mb-6">
        {STEPS.map((s, i) => (
          <React.Fragment key={i}>
            ...
            {i < STEPS.length - 1 && (
              <div className="flex-1 h-px" style={{ background: i < step ? 'var(--teal-600)' : 'var(--border-input)' }} />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="space-y-4">
          <Field label="Full Name" k="name" icon={User} placeholder="Rahul Sharma" required />
          <Field label="Email Address" k="email" type="email" icon={Mail} placeholder="rahul@college.edu" required />

          <div>
            <label className="form-label">Password<span className="text-teal-400 ml-0.5">*</span></label>
            <div className="relative">
              <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: 'var(--text-label)' }} />
              <input
      ...
      {step === 1 && (
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <Field label="College / University" k="college" icon={School}
            placeholder="e.g. IIT Bombay, VIT Vellore" required />

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="form-label">Degree<span className="text-teal-400 ml-0.5">*</span></label>
            ...
            <Field label="Branch / Stream" k="branch" icon={GitBranch} placeholder="e.g. CSE, ECE, MBA" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="form-label">Year of Study</label>
              <select value={form.yearOfStudy} onChange={e => set('yearOfStudy', e.target.value)} className="input-field">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Student / Roll No." k="studentId" icon={Hash} placeholder="e.g. 21CS1001" />
            <Field label="Graduation Year" k="graduationYear" icon={Calendar} placeholder="e.g. 2027" />
          </div>

          <div className="flex gap-3 pt-1">
            <button type="button" onClick={() => setStep(0)} className="btn-secondary flex-1 h-11">← Back</button>
